...
const setup = cli => {
  // Load default config
  nconf.file(__dirname + '/config/defaultConfig.json')
  ...
  // What the hell is going on here? Well, we basically need to fool
  // panoptes-client into choosing the right API endpoint. We could set it
  // manually, but it should be up to the client, really.
  ...
  try {
    const userConfig = JSON.parse(fs.readFileSync(os.homedir() + '/.annotate_uploader', 'utf8'));
